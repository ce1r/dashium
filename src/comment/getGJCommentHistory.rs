...
use cornucopia::queries::comment::get_comment_history;
...
pub async fn getGJCommentHistory(Form(form): Form<Data>) -> Result<impl IntoResponse> {
    let client = Database::acquire().await?;
    let offset = form.page * 10;

    let comments = get_comment_history()
        .bind(&client, &form.userID, &offset)
        .all()
        .await?;
